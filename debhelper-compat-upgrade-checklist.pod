...
The B<dh_assistant compat-upgrade-checklist> command can provide a package specific upgrade
checklist from compat 7 and up (requires B<debhelper/13.25> or later). Its main limitation
is accuracy of its heuristics. Please review the documentation of L<dh_assistant(1)> for
details.
...
source packages that have a single B<Package> stanza in F<debian/control>. If
the package needs the short-cuts for single-binary packages, it must explicitly
activate the B<single-binary> add-in.

This can be done via a B<Build-Depends> on B<dh-sequence-single-binary>.

Any B<--without single-binary> passed to B<dh> to avoid the warning in compat 14
can now be removed to simplify F<debian/rules> without triggering the warning.
...
The B<dh_installmodules> tool will now install its
configuration files under F<< /usr/lib/modprobe.d/I<package> >>
instead of F<< /etc/modprobe.d >>.

Please consider using the "rm_conffile" feature from L<dh_installdeb(1)> to
ensure the proper removal of previous modprobe files.

Note: Retroactively change in B<debhelper/14.1> - The B<dh_installpam>
command had a similar change, which was reverted at the request of the
B<pam> maintainer.
...
The B<dh_dwz> command has been removed from the default sequence. If it is still desired,
add B<dh-sequence-dwz> to B<Build-Depends-Arch> or run it in
B<execute_before_dh_strip-arch> with B<-a> to get the closest emulation of the previous
behavior.

Note: The B<dh_dwz> command will cause a lot of differences in the generated debug symbols.
If you use B<diffoscope> as part of your compat upgrade checks, you may want to temporarily
use B<dh_dwz> to keep the delta reviewable while upgrading and then remove B<dh_dwz>
afterwards.
...
A new tool called B<dh_computeautosubstvars> has been added and is now run immediately
before B<dh_gencontrol>. If you inject commands before B<dh_gencontrol>
(either via hook targets or third-party sequence add-on), consider moving them
to after B<dh_installdeb> instead as the new tool generally assumes no more changes
will occur between it is called and B<dh_gencontrol> is called.

Notable exception to this assumption if you have to tweak substvars per
L<https://lists.debian.org/debian-devel/2024/03/msg00030.html>.

=item -

I<< This item only applies to source packages that have exactly one B<Package> stanza in F<debian/control>. >>

The B<dh_auto_install> command now unconditionally uses B<--destdir=debian/tmp> by default. The special-case
for source packages building a single binary package is now moved to B<single-binary> B<dh> addon. Note, this
add-on is activated by default in compat 14 but not in compat 15 (see next bullet item).
...
warn maintainers of the pending compat 15 change in B<dh_auto_install>. The implicit
activation is a transitional feature to reduce the changes of risks with this change.
In compat 15, the implicit activation will no longer trigger.
...
the package. Units that should not be started/stopped/restarted automatically should
be marked with the B<RefuseManualStart=yes> and/or B<RefuseManualStop=yes> unit directives.

=item -

The B<dh_gencontrol> tool now automatically applies relationship substvars to
...
field (e.g., B<${*:Depends}>, B<${*:Recommends}>). At the time of writing, the
list consists of:
...
L<lintian(1)>. Please check L<https://bugs.debian.org/1067653> for
L<lintian(1)> support for this change.

Finally, B<debhelper> also applies this feature the substvars for the
following fields due to implementation details:
...
See L<#638517|https://bugs.debian.org/638517> for what the field is used for.
...
Fixed in B<debhelper/14.3>: There was a bug in the implementation that meant it
did not trigger for compat 14 before B<debhelper/14.3>.

=item -

When running B<dh_auto_install>, B<debhelper> provided build systems will now ensure
all paths in the B<destdir> have minimal user permissions (B<chmod -R u+rwX>) to avoid
weird I<permission denied> errors during builds.

Third-party provided debhelper build systems are recommended to support this as well.
This can be done by running

     $this->ensure_minimal_permissions($destdir) if not compat(13);

in the B<debhelper> Buildsystem code from their B<sub install> implementation after
the upstream code has been run.

If you are not using B<dh_auto_install> and you run into weird I<permission denied>
errors, you can often solve this by running B<chmod -R u+rwX DIRECTORY> after running
the install target from the upstream build system.

Please take care to check if your package has any special cases for permissions where
files or directories for some reason must not have the "user write bit" set. Known
cases are B<*.ali> files and B</etc/sudoers.d>, which B<dh_fixperms> will fix up by
default.

The problem with uncommon permissions have always been present in theory. However, it
has become considerable more visible with B<dh_fixperms> being run later in the
sequence as of compat 14 and with the move to remove B<fakeroot> by default (which
papered over some of these issues). This change is aimed at mitigating the problem.
...
For most packages, this item can be fixed by running:

     apt-get satisfy 'debputy (>= 0.1.73~), debhelper (>= 13.26~)'
     debputy migrate-from-dh --migration-target=dh-package-prefixed-config-files

The only known caveat are architecture specific configuration files (such as
F<debian/symbols.amd64> or F<debian/install.amd64>).

=item -

It now triggers a warning to use a packaging file without the package prefix for
B<--name> even if the source package only produces one binary package. As example,
if you had a F<debian/bar.service> with the following snippet in F<debian/rules>:
...
The only known caveat are architecture specific configuration files. The B<debhelper>
stack does not accept any config files that have architecture restrictions B<and>
B<--name>. However, third-party add-ons might.

=item -

The default look up rules for B<Dh_Lib> based tools now assume that configuration
files are no longer named (B<--name>) nor support architecture restrictions by default.
If you work with a third-party debhelper-like tool and need support for either of these
features, please file a bug against the tool asking it to declare its configuration
file with the relevant options in its B<pkgfile> call.

Note that B<debhelper> itself tweaked its rules for most of its tools as well based on
analysis of usage via B<codesearch.debian.org>. Should you be relying on a feature like
...
debhelper compat level. Please review L<debhelper(7)/COMPATIBILITY LEVELS>
for alternatives.

=item -

The tool B<dh_installtmpfiles> now runs with B<--remove> on package removal, and
B<--purge> on package purge. systemd v256 is required for the latter.

=item -

The B<dh_lintian> tool no longer accepts architecture specific overrides files
for packages with B<Multi-Arch: same> in F<debian/control>, since these would
not be co-installable. Instances affected by this error should migrate to
L<lintian(1)> architecture specific overrides.

=item -

The third-party B<dh_qmldeps> tool (from B<pkg-kde-tools> package) no longer
demote detected dependencies. In v13 you get warnings about demotions. You can
use the B<-d dependency field> in the B<debian/PKG.qmlfiles> to get the
dependencies end up in the correct B<dependency field>.
...
The B<dh_auto_*> helpers now reset the environment variable B<HOME>
and the common B<XDG_*> variables.  Please see description of the
...
B<-I.> to perl.  Packages that rely on this behavior can often use the
...
The B<--doc-main-package> option (or, since debhelper/13.28, the
B<X-Doc-Main-Package> field) can be used when the auto-detection
