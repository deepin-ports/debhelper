...
package.  This is so they can locate files like F<debian/control>
when needed.
...
with an error if the token contains many substitution variables (50) or
the total size of the expanded value exceed a certain size (4096
characters or 3x length of the original input - whichever is bigger).

Note the limit is per token. Every helper decides its own definition
of a token. The common case of files like F<< debian/I<package>.install >>,
each token is a file/path name.
...
(or, in the long term, to yourself). Setting a global B<DH_COMPAT>
...
When inserting the B<#DEBHELPER#> token into a script, consider whether
any of the hand written code has a compelling reason to appear I<before>
the B<#DEBHELPER#> token. The B<#DEBHELPER#> token will generally contain
the code that start (F<postinst>) or stop (F<prerm>/F<preinst>) any
services the package is providing. Often (but not always), it is better
to let the code from B<#DEBHELPER#> run first. Common exceptions include
things like B<set -e>, initializing B<debconf> (when needed) and
special-cases like generation or hand-written management of configuration
files. These things should generally occur before the B<#DEBHELPER#>
token.
...
=head2 Automatic injection of substvar dependencies.

Starting with compat 14, B<dh_gencontrol> will automatically apply relationship
substvars to relevant fields. That means that many substvars such as
B<${misc:Depends}> and B<${shlibs:Depends}> no longer need to be explicitly
mentioned in F<debian/control>. This applies to any substvar named after a
field that the installed version of B<dpkg> considers a relation or
dependency-like field. For example: B<${*:Pre-Depends}>, B<${*:Depends}>,
B<${*:Recommends}>, B<${*:Conflicts}>, B<${*:Breaks}>, B<${*:Replaces}>,
B<${*:Provides}>, B<${*:Built-Using}>, B<${*:Static-Built-Using}>, etc.
...
dependencies for you, since the dependency is done via a "relationship
substvar" (see L<https://lists.debian.org/debian-devel/2024/02/msg00230.html>
and L<https://lists.debian.org/debian-devel/2024/03/msg00030.html> for details).

In earlier compat levels, you have to manually specify the relevant substvars
in the relevant fields. Typically, B<${misc:Depends}> for the B<Depends>
field and B<${misc:Pre-Depends}> for the B<Pre-Depends> fields.

The variables listed here are releated to miscellaneous dependencies
(B<${misc:*}>) and are entirely independent of the standard
B<${shlibs:Depends}> generated by L<dh_makeshlibs(1)>, and the
B<${perl:Depends}> generated by L<dh_perl(1)>. For compat 13 and earlier you
will also have to add these to the respective fields when relevant.
